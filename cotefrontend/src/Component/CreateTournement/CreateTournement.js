...
// console.log(JSON.stringify(game))
  ...
  useEffect(() => {
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      if (image !== null) { post.append('photo', image.file); }
    ...
    <Grid
      ...
      <FormControl sx={{ m: 1, width: '52ch' }} >
        <Autocomplete
          ...
          value={values.jeux}
          options={[...gamesapi.map(item => item.name)].sort((a, b) => a.localeCompare(b))}
          sx={{ width: '52ch' }}
      ...
      <FormControl required sx={{ m: 1, width: '52ch' }} variant="outlined">
        <InputLabel htmlFor="name" style={{ color: 'rgb(159, 156, 156)' }}>Discord Link</InputLabel>
        <OutlinedInput
          name="link"
          onChange={handleChange}
          value={values.link}
