...
import { editUser, getUser, getUserFollowers, getUserFollowing } from '../../../Services/UserService';
...
function Profilee() {
    ...
    const handleFileChange = (e) => {
        ...
        if (photo) {
            ...
            axios.post("http://127.0.0.1:5000/user/addprofilephoto", formData, {
                headers: {
                ...
                .then((response) => {
                    ...
                    setImageSrc(`http://localhost:5000/uploads/user/${response.data.photo}`);
        ...
        <Grid container>
            <Grid item direction="row" >
                ...
                <div style={{ marginLeft: '5vw', width: "45vw", marginTop: '5vh' }}>
                    <div style={{ display: "flex", flexDirection: "row", fontSize: '3vh', fontFamily: 'serif' }}>
                        <h2 style={{ color: '#343beb', marginRight: "3vw" }}>First Name </h2>
                    ...
                    {values && password && (
                        <div>
                            ...
                            >
                                <FaPencilAlt style={{ marginLeft: '-5px', marginRight: '5px' }} />
                                Edit Profile
                            </Button>
                            <Dialog open={open} onClose={handleClose}
                                ...
                                <DialogContent>
                                    ...
                                    >
                                        <img src={imageSrc} alt="pic" id="photo" />
                                        <input type="file" id="file" onChange={handleFileChange} />
                                        {displayUploadBtn && (
                                            <label htmlFor="file" id="btnchoose">
                                                <FaPencilAlt /> Edit photo
                                            </label>
                                        )}
                                    </div>
                                    <label htmlFor="name" style={{ color: '#fff', }}>First Name</label>
                                    <InputText id="firstname" name="firstname" value={values.firstname} onChange={handleEdit} style={{ border: '1px solid #343beb' }} />
                                  {/* //herree */}
                                    {formErrors.firstname && (
                                        <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.firstname}</p>
                                    )}

                                    <label htmlFor="lastname" style={{ color: '#fff' }}>Last Name</label>
                                    <InputText id="lastname" name="lastname" value={values.lastname} onChange={handleEdit} style={{ border: '1px solid #343beb' }} />
                                    {formErrors.lastname && (
                                        <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.lastname}</p>
                                    )}

                                    <label htmlFor="email" style={{ color: '#fff' }}>Email</label>
                                    <InputText id="email" name="email" value={values.email} style={{ border: '1px solid #343beb' }} />
                                    {/* {formErrors.email && (
                                                <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.email}</p>
                                            )} */}

                                    <label htmlFor="currentpassword" style={{ color: '#fff' }}>Current Password</label>
                                    <InputText id="currentpassword" name="currentpassword" onChange={handleEditPassword} style={{ border: '1px solid #343beb' }} />
                                    {formErrors.currentpassword && (
                                        <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.currentpassword}</p>
                                    )}

                                    <label htmlFor="newpassword" style={{ color: '#fff' }}>New Password</label>
                                    <InputText id="newpassword" name="newpassword" onChange={handleEditPassword} style={{ border: '1px solid #343beb' }} />
                                    {formErrors.newpassword && (
                                        <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.newpassword}</p>
                                    )}

                                    <label htmlFor="confirmpassword" style={{ color: '#fff' }}>Confirm New Password</label>
                                    <InputText id="confirmpassword" name="confirmpassword" onChange={handleEditPassword} style={{ border: '1px solid #343beb' }} />
                                    {formErrors.confirmpassword && (
                                        <p style={{ color: 'red', fontSize: '2.5vh', marginTop: "0.5vh", marginLeft: "-1vw" }}>{formErrors.confirmpassword}</p>
                                    )}
                                </DialogContent>


                                <DialogActions>
                                    <Button onClick={handleSaveandClose} style={{ color: "white", backgroundColor: "#343beb" }}>Save Changes</Button>
                                    <Button onClick={handleClose} style={{ color: "black", backgroundColor: "white" }}>Cancel</Button>
                                </DialogActions>
                            </Dialog>
                        </div>
                    )}
