...
function Resetpass() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (expirationDate && new Date() > new Date(expirationDate)) {
        ...
        setExpirationData({ expired: true });
      }
    }, 1000);
  ...
  }, [formErrors, isSubmit, values, id, token, navigate]);
  ...
  return (
    ...
    >
      ...
      <FormControl required sx={{ m: 1, width: "52ch" }} variant="outlined">
        <InputLabel htmlFor="password" style={{ color: "rgb(159, 156, 156)" }}>
          New password
        </InputLabel>
        <OutlinedInput
          name="password"
...
export default withScrollToTop(Resetpass);
