        ...
        {isLogged ? (
          <Box className='comp-box' >
            {pages.slice(1).map((page, index) => {
              ...
              return (
                <NavLink style={{ textDecoration: 'none' }} to={`/${page.toLowerCase().replace(/\s/g, '-')}`} key={index}>
                  <Button
                    key={page}
          ...
          <Box display="flex" alignItems="center">
            ...
            {isAdmin ? (<></>)
              : (<Stack direction="row" className='av'>
                <Link to="/createlive" style={{ textDecoration: 'none', color: '#343beb', }}>
                  <StyledBadge
                    overlap="circular"
                    anchorOrigin={{ vertical: 'top', horizontal: 'right' }}
                    variant="dot"
                  >
                    <img src={`${streamicon}`} alt='streamicon' className='streamicon' />
                  </StyledBadge>
                </Link>
              </Stack>)}
            ...
            <Box sx={{ flexGrow: 0 }}>
              <Tooltip className='tool'>
                <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                  <Avatar alt="Remy Sharp" src={`http://localhost:5000/uploads/user/${data.photo}`} />
                </IconButton>
              </Tooltip>
              <Menu
                sx={{ mt: '45px' }}
              ...
              >
                <div className='NAMECard'>
                  <img src={`http://localhost:5000/uploads/user/${data.photo}`} alt="Photo de profil" className='pp' />
                  <div className='CARDname'>{data.firstname}</div>
                  <div className='CARDlastname'>{data.lastname}</div>
                </div>
                {isAdmin ? (
                  admin.map((adminItem, index) => (
                    <MenuItem key={index} onClick={() => handleAdminItemClick(adminItem)}>
                      {adminItem === "Log out" && (
                        <Typography textAlign="center" style={{ flex: '1' }}>
                          <LogoutIcon style={{ marginLeft: '1.2rem' }} />{adminItem}
                        </Typography>
                      )}
                      {adminItem !== "Log out" && (
                        <Typography textAlign="center">{adminItem}</Typography>
                      )}
                    </MenuItem>
                  ))
                ) : (
                  settings.map((setting, index) => (
                    <MenuItem key={index} onClick={() => handleSettingClick(setting)}>
                      {setting === "Log out" && (
                        <Typography textAlign="center" style={{ flex: '1' }}>
                          <LogoutIcon style={{ marginLeft: '1.2rem' }} />{setting}
                        </Typography>
                      )}
                      {setting !== "Log out" && (
